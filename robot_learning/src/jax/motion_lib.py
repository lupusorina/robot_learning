...
import motion.py
import torch_util as torch_util

# check about logger and see what it can be used.
...
class MotionLib():
    # motion file, the pkl file with the motion frames
    # kin_char_model: the skeleton of the motion
    # device: gpu used for the work.
    def __init__(self, motion_file, kin_char_model, device):
        self_device = device
        self._kin_char_model_ = kin_char_model
        self._load_motions(motion_file)
        return
    
    def get_num_motions(self):
        # theres an enum ahead
    ...
    # it fetches a random timepoint from the motion frame.
    #the goal of random sampling is for better robustness.
    ...
    # converts absolute motion times into normalized phase values.
    # handles lloop modes by wrapping phase values that exceed 1.0
    # for deepmimic is part of the observation vector, so the policy knows 
    # where it is in the motion cycle
    ...
    # interpolation between two motion frames to produce a pose at a specific time.
    # what is interpolation? inserting of an intermediate value from surrounding known values.
    #
    def calc_motion_frame(self, motion_ids, motion_times):
        ...
        blend_unsq = blend.unsqueeze(-1) #adding a dimension at the last position for broadcasting

        # each variable is an interpolation between two frames
    ...
    def _extract_frame_data(self, frame):
        ...
        # coverts DOF to quaternions
        joint_rot = self._kin_char_model.dof_to_rot(joint_dof)
        # normalize quaternions to have a positive w component.
        joint_rot = torch_util.quat_pos(joint_rot)
        #the lines above helps to prevent issues in interpolation and comparisons

        return root_pos, root_rot_quat, joint_rot


    # finding the two nearest frames for interpolation
    # and computes the blend factor.
    # it helps to determine which frames to interpolate between.
    ...
    # computing the position offset when a motion loops multiple times(WRAP MODE)
    # WRAP Mode: it repeats from the start.
    #in our case, when the motion moves forward, each loop 
    # should accumulate position.
    #it returns the position offset to add to the interpolated root position.
    #used in calc_motion_frame()
    ...
    def _load_motions(self, motion_file):
        ...
        print(f"Loaded motion file {motion_file}")
        print(f"Loaded {num_motions:d} motions with a total length of {total_len:.3f}s.")
        return
    ...
    def _load_motion_pkl(self, motion_file):
        ...
        for f in range(num_motion_files):
            ...
            print(f"Loading {f+1:d}/{num_motion_files:d} motion_files: {curr_file:s}")
    ...
    def _fetch_motion_files(self, motion_file):
        ext = os.path.splittext(motion_file)[1]
        # it uses yaml mimickfile.
        if (ext==".yaml"):
            motion_files = []
            motion_weights = []
